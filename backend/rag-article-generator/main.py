...
def main():
    ...
    # MODE ARTICLE - NOUVEAU PIPELINE
    # print("\n=== GENERATION DU CANVA ===")
    # canva = generate_article_canva(llm, args.question)
    # print(canva)
    # print("===CANVA_JSON_START===")
    # print(json.dumps(canva, ensure_ascii=False, indent=2))
    # print("===CANVA_JSON_END===")
    ...
    #     print("Réponse générée :", data["generated_answer"])
    #     generate_matplotlib_figure(data["content_type"], data["question"])
    ...
    # # 🔧 Fusionner la question initiale + les réponses dans le canva
    # canva["original_question"] = args.question
    # for key in ["part1", "part2", "part3"]:
    #     if key in canva and key in results:
    #         # garder la question telle que calculée par le graphe
    #         canva[key]["question"] = results[key].get("question", canva[key].get("content", ""))
    #         # injecter la réponse générée pour affichage frontal
    #         canva[key]["generated_answer"] = results[key].get("generated_answer", "")

    # # 🔧 Imprimer un seul bloc JSON final à consommer par app.py
    # print("\n===CANVA_JSON_START===")
    # print(json.dumps(canva, ensure_ascii=False, indent=2))
    # print("===CANVA_JSON_END===")

    # # (Optionnel) conserver les logs lisibles ensuite si tu veux
    # print("\n=== RÉSULTATS ===")
    # for part, data in results.items():
    #     print(f"\n--- {part.upper()} ---")
    #     print("Question :", data["question"])
    #     print("Réponse générée :", data["generated_answer"])
    #     # generate_matplotlib_figure(data["content_type"], data["question"])
    #     generate_mui_chart_component(data["content_type"], data["question"])
    ...
    # if graphs:
    #     canva["graphs"] = graphs  # ex: { "part1": { jsx: "..."} , ... }

    # # 🔧 Un seul bloc JSON final (rétro-compatible)
    # print("\n===CANVA_JSON_START===")
    # print(json.dumps(canva, ensure_ascii=False, indent=2))
    # print("===CANVA_JSON_END===")
