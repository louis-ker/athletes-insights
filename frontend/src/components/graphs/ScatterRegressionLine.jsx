...
//               markerSize: 2,
//               valueFormatter: (v) => `${v.y} y for ${v.x} x`,
//             },
//           ]}
//         >
...
export default function ScatterRegressionLine({ className }) {
  ...
  // Hauteur du chart calculée à partir de la hauteur du container
  const chartHeight = React.useMemo(() => {
    if (!containerHeight) return 220; // valeur par défaut confortable
    const reserved = 60; // ~ espace pour le titre + caption
    return Math.max(containerHeight - reserved, 140);
  }, [containerHeight]);
